      ...
      if (!artNrSameFlag) {
        async function asyncCall() {
          const [responseSaveArticle] = await Promise.all([
            $api.outlet.getOUAction('complimentReportChgArtnr', {
              cListPArtnr: state.article['value'],
              cListBezeich: state.dataRowSelected['bezeich'],
              artnr: state.dataRowSelected['p-artnr'],
          ...
          if (responseSaveArticle) {
            ...
            const flagSave = responseSaveArticle['availHArtikel'];
            
            if (flagSave) {
              if (!nameSameFlag) {
                async function asyncCall() {
                  const [responseSaveName] = await Promise.all([
                    $api.outlet.getOUAction('complimentReportChgName', {
