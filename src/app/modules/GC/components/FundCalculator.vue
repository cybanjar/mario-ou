  ...
  <div>
    <q-dialog v-model="fund_calculator.dialog" persistent>
      <q-card style="width: 950px; max-width: 90vw; height: 500px">
        ...
        <q-card-section
          style="marginTop: 15px; height: 390px"
          class="q-pt-none row"
        >
          <div class="col-5 row">
            <SSelect
            ...
            <SInput
              ...
              :disable="x.disable"
              v-for="x in use_input.filter((x) =>
                ['Account Number'].includes(x.name)
              )"
              :key="x"
            />
            <q-btn
            ...
            <SRemarkLeftDrawer
              v-for="x in use_input.filter((x) =>
                ['Total Debit', 'Balance'].includes(x.name)
              )"
              :key="x.name"
            ...
            <FormatMoneyInput
              ...
              v-for="x in use_input.filter((x) =>
                [
                  ...
                  'Requested Ending Balance',
                  'Cheque/Giro To Be Opened',
                ].includes(x.name)
              )"
            />         
          </div>
...
import FormatMoneyInput from './common/FormatMoneyInput.vue';
...
export default defineComponent({
  ...
  setup(props) {
    ...
    const onReserved = (params) => {
      const number = params.value.replace(/,/g,'')
      if (!isNaN(number)) {
        if (params.onCLick == 'reserved') {
          let hasil =
            Number(use_input[2].value.replace(/,/g, '')) -
            Number(use_input[3].value.replace(/,/g, ''));
          hasil2 = Number(number) + hasil;
        ...
        if (params.onCLick == 'Requested') {
          let hasilGiro = hasil2 + Number(number);
  ...
  components: {
    ModalFundCalculatorAdd: () =>
      import('./childComponents/ModalFundCalculatorAdd.vue'),
      FormatMoneyInput
  },
});
</script>
