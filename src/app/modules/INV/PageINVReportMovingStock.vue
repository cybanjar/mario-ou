...
import { tableHeaders } from './tables/movingStock.table';
...
export default defineComponent({
  setup(_, { root: { $api } }) {
    ...
    const onSearch = (state2) => {
      ...
      if (state2.date == null) {
        ...
        async function asyncCall() {
          const response = await $api.inventory.FetchAPIINV('stockOhlistList', {
              ...
              moveArt: state2.shape,
            }),
            charts = response['stockohList']['stockoh-list'] || [];
          ...
          for (let i = 0; i < charts.length; i++) {
            ...
            dataRow['prevqty'] = formatterMoney(dataItem['prevqty']);
            dataRow['prevval'] = formatterMoney(dataItem['prevval']);
            dataRow['incoming'] = formatterMoney(dataItem['incoming']);
            dataRow['outgoing'] = formatterMoney(dataItem['outgoing']);
            dataRow['actqty'] = formatterMoney(dataItem['actqty']);
            dataRow['actval'] = formatterMoney(dataItem['actval']);
            dataRow['avrg-pr'] = dataItem['avrg-pr'];
    ...
    function doPrint() {
      if (state.data.length !== 0) {
        PrintJs(state.data, tableHeaders, 'Moving Stock');
      }
    }
    return {
      ...toRefs(state),
  ...
  components: {
    SearchMovingStock: () => import('./components/SearchMovingStock.vue'),
  },
});
</script>
