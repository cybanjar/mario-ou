<template>
  <div>
    ...
    <div class="q-pa-lg">
      ...
      >
        <template #header="props">
          <q-tr :props="props">
            <q-th :props="props" key="articelNumber" rowspan="2">{{
              props.colsMap.articelNumber.label
            }}</q-th>

            <q-th :props="props" key="descriPtion" rowspan="2">{{
              props.colsMap.descriPtion.label
            }}</q-th>
            <q-th colspan="2">Mess</q-th>
            <q-th colspan="2">Delivery</q-th>
            <q-th :props="props" key="masseinheit" rowspan="2">{{
              props.colsMap.guestNote.label
            }}</q-th>
            <q-th colspan="3">Price</q-th>
            <q-th :props="props" key="purchase" rowspan="2">{{
              props.colsMap.purchase.label
            }}</q-th>
            <q-th :props="props" key="articelNumber" rowspan="2"
          ...
          >
            <q-td
              ...
              v-for="col in props.cols.filter(
                (cols) => !['actions'].includes(cols.name)
              )"
              :key="col.name"
            >
              {{ col.value }}
    ...
    <DialogChartOfAccounts :data-dialog="dataDialog" />
  </div>
</template>
...
export default defineComponent({
  setup(_, { root: { $api } }) {
    ...
    // FETCH API
    const FETCH_API = async (api, body?, xi?) => {
    ...
    const onScroll = ({ index }) => {
      ...
      if (index == lastIndex) {
        ...
        FETCH_API('getInvArticleList', data, 'xi');
      }
    };
    ...
    function doPrint() {
      if (state.data.length !== 0) {
        PrintJs(state.data, roomTableHeaders, 'Stock Item');
      }
    }
    ...
    function getDefaultData(cols) {
      return cols.filter(
        (col) =>
          !['articelNumber', 'descriPtion', 'accountNumber'].includes(col.name)
      );
    }
    return {
      ...toRefs(state),
