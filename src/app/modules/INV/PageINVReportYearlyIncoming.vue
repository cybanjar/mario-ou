<template>
  <div id="app">
    <q-drawer :value="true" side="left" bordered :width="250" persistent>
      <SearchYearlyIncoming :searches="searches" @onSearch="onSearch" />
    </q-drawer>
...
export default defineComponent({
  setup(_, { root: { $api } }) {
    ...
    onMounted(async () => {
      const [resDepart] = await Promise.all([
        $api.inventory.FetchAPIINV('rcvAnnualPrepare'),
      ]);

      state.ciDate = resDepart.ciDate;
    ...
    function doPrint() {
      if (state.data.length !== 0) {
        PrintJs(state.data, tableHeaders, 'Yearly Incoming');
      }
    }
    ...
    const onSearch = (state2) => {
      ...
      async function asyncCall() {
        const response = await $api.inventory.FetchAPIINV('rcvAnnualList', {
            pvILanguage: 1,
  ...
  components: {
    SearchYearlyIncoming: () => import('./components/SearchYearlyIncoming.vue'),
  },
});
</script>
