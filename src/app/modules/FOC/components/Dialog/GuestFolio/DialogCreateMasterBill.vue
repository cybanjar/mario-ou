<template>
  <q-dialog v-model="getDialogCreateMasterBill">
...
export default defineComponent({
  setup(props, { root: { $api } }) {
    ...
    const getDialogCreateMasterBill = computed(() => {
      return store.getters.focGuestFolio.GET_DIALOG_CREATE_MASTER_BILL;
    });
    ...
    const onClickOk = () => {
      ...
      store.commit.focGuestFolio.SET_DIALOG_CREATE_MASTER_BILL(false);
    };

    return {
      getDialogCreateMasterBill,
      getReadMasterBill,
      getReadGuest,
      onClickOk,
      ...toRefs(state),
