...
export function reformLedgerData(temp: LedgerRes[]): LedgerData[] {
  ...
  const data = temp.map<LedgerData>((temp, index) => ({
    ...
    description: temp.bezeich.replace(
      /T O T A L|GRAND T O T A L/gi,
      function (matched) {
...
export function toSecond(format: string): number {
  const [hours, minutes, seconds] = format.split(':');
  const hSecond = parseInt(hours) * 3600;
  const mSecond = parseInt(minutes) * 60;

  return hSecond + mSecond + parseInt(seconds);
}
